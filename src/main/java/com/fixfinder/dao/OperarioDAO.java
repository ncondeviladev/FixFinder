 ...
 */
public class OperarioDAO implements BaseDAO<Operario> {
    ...
    public void insertar(Operario operario) throws DataAccessException {
        ...
        try {
            ...
            // Por defecto, JDBC guarda cada sentencia inmediatamente (AutoCommit = true).
            // Lo desactivamos para que nada sea definitivo hasta que nosotros digamos
            // 'commit()'.
            // Esto es vital: si falla la segunda inserción, no queremos que se guarde la
            // primera.
            ...
            try (PreparedStatement stmtUser = conn.prepareStatement(sqlUsuario, Statement.RETURN_GENERATED_KEYS)) {
                ...
                try (ResultSet generatedKeys = stmtUser.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                        operario.setId(generatedKeys.getInt(1));
                    } else {
            ...
            // Usamos el ID que acabamos de recuperar (operario.getId()) como clave primaria
            // y foránea.
            try (PreparedStatement stmtOp = conn.prepareStatement(sqlOperario)) {
                ...
                stmtOp.setString(3, operario.getEspecialidad());
            ...
            // Si hemos llegado hasta aquí, ambas inserciones han funcionado.
            // Guardamos los cambios definitivamente en la base de datos.
            conn.commit();

        } catch (SQLException e) {
            // --- GESTIÓN DE ERRORES (ROLLBACK) ---
            // Si algo falla en el camino (ej: DNI duplicado, fallo de red...),
            // deshacemos todo lo que hayamos hecho en esta transacción.
            // Así evitamos dejar usuarios "huerfanos" sin datos de operario.
            if (conn != null) {
                try {
            ...
            // --- LIMPIEZA ---
            // Restauramos el modo AutoCommit por defecto para no afectar a otras
            // operaciones
    ...
    public void actualizar(Operario operario) throws DataAccessException {
        ...
        try {
            ...
            try (PreparedStatement stmtOp = conn.prepareStatement(sqlOperario)) {
                ...
                stmtOp.setString(2, operario.getEspecialidad());
