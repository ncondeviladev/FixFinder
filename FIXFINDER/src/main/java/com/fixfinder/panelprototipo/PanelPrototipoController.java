...
public class PanelPrototipoController {
    ...
    private void onConectarClick() {
        try {
            if (!servicioCliente.isConectado()) {
                log("Conectando a " + GlobalConfig.getServerIp() + "...");
    ...
    private void mostrarDetalleTrabajo(TrabajoModelo t) {
        ...
        // Sección de GESTIÓN (Solo para GERENTES)
        if ("GERENTE".equalsIgnoreCase(usuarioLogueadoRol)) {
            content.getChildren().add(new Label("Operaciones de Gerencia:"));
            VBox boxGestion = new VBox(5);
            boxGestion.setStyle("-fx-border-color: #ccc; -fx-padding: 10; -fx-background-color: #f4f4f4;");

            // OPCIÓN 1: PRESUPUESTAR (Solo si está PENDIENTE o PRESUPUESTADO)
            if ("PENDIENTE".equals(t.getEstado()) || "PRESUPUESTADO".equals(t.getEstado())) {
                HBox boxPresu = new HBox(10);
                TextField txtMonto = new TextField("100.0");
                txtMonto.setPrefWidth(60);
                Button btnPresu = new Button("Enviar Presupuesto (€)");
                btnPresu.setOnAction(e -> {
                    try {
                        ...
                        new Alert(Alert.AlertType.ERROR, "Monto inválido o error de red").show();
                    }
                });
                boxPresu.getChildren().addAll(txtMonto, btnPresu);
                boxGestion.getChildren().add(new Label("Paso 1: Presupuestar"));
                boxGestion.getChildren().add(boxPresu);
            }

            // OPCIÓN 2: ASIGNAR (Solo si está ACEPTADO o ya ASIGNADO)
            if ("ACEPTADO".equals(t.getEstado()) || "ASIGNADO".equals(t.getEstado())) {
                boxGestion.getChildren().add(new Label("Paso 2: Asignar Operario"));
            ...
            } else if (!"PENDIENTE".equals(t.getEstado()) && !"PRESUPUESTADO".equals(t.getEstado())) {
                // Si no está ni para presupuestar ni para asignar ni aceptado, mostramos que
                // está en proceso
                boxGestion.getChildren().add(new Label("Trabajo en estado: " + t.getEstado()));
            } else {
                boxGestion.getChildren().add(new Label("Pendiente de que el cliente ACEPTE un presupuesto."));
            }
