        ...
        btnActualizar.setOnAction(e -> {
            ...
            new Thread(() -> {
                ...
                Platform.runLater(() -> {
                    ...
                    lblEstadoAccion.setText("Tabla actualizada.");
                });
            }).start();
        });
            ...
            try {
                ...
                if (respuesta != null && respuesta.get("status").asInt() == 200) {
                    ...
                    Platform.runLater(() -> {
                        ...
                        alert.setHeaderText(
                                "El servidor devolvió un error (Status " + respuesta.get("status").asInt() + ")");
                        alert.setContentText(respuesta.get("mensaje").asText());
                        alert.show();
                    });
                }
            } catch (Exception e) {
    ...
    private JsonNode enviarSolicitud(ObjectNode requestJson) {
        try (Socket socket = new Socket(GlobalConfig.getServerIp(), GlobalConfig.PORT);
            ...
            Platform.runLater(() -> {
                ...
                errorAlert.setHeaderText("No se pudo conectar con el Servidor Central");
