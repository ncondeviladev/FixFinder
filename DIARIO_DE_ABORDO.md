...
> **âš ï¸ Nota:** La carpeta `DOCS/` **sí se sube a Git** pero el repositorio de GitHub debe mantenerse **privado** para que la Memoria del proyecto no sea pública.
...
- **Backend:** Servidor Java puro con Sockets TCP en puerto `5000`. Sin Spring Boot.
  ...
  - **Protocolo de Comunicación:** 4 bytes de cabecera (longitud del mensaje) + payload JSON en bytes.
    ...
    - **âš ï¸ El simulador `SimuladorController.java` usa también el protocolo de 4 bytes (ya actualizado)**
...
> âš ï¸ IMPORTANTE: Los tests de JUnit (`ServiceTest`) generan usuarios temporales en la BD y pueden dejar telefono=NULL en usuarios existentes. Después de correr tests, ejecutar:
>
> ```sql
...
- `solicitarReparacion(idCliente, titulo, categoria, descripcion, direccion, urgencia)` — Crea trabajo PENDIENTE
  - **âš ï¸ Nuevo (01/03):** Si `direccion` viene vacío, usa `cliente.getDireccion()` como fallback. Si tampoco tiene, pone "Sin dirección especificada"
...
> âš ï¸ IMPORTANTE sobre `procesarValorarTrabajo`: El mensaje de éxito en el JSON de respuesta es
...
````
lib/
├── main.dart                          â† Entrada, providers, rutas, tema
├── models/
│   ├── trabajo.dart                   â† Modelo Trabajo + enums EstadoTrabajo, CategoriaServicio
│   ├── usuario.dart                   â† Modelo Usuario + enum Rol
│   ├── presupuesto.dart               â† Modelo Presupuesto
│   └── empresa.dart                   â† Modelo Empresa colaboradora
├── providers/
│   └── trabajo_provider.dart          â† State management para trabajos
├── services/
│   ├── socket_service.dart            â† Comunicación TCP con servidor Java (protocolo 4 bytes)
│   └── auth_service.dart             â† Login, logout, persistencia token en SharedPreferences
├── screens/
│   ├── login_pantalla.dart
│   ├── dashboard_pantalla.dart
│   ├── detalle_trabajo_pantalla.dart  â† REFACTORIZADA: delega en widgets separados
│   ├── crear_trabajo_pantalla.dart    â† Crear y Modificar (modo dual)
...
└── widgets/
    ...
    │   └── galeria_fotos.dart         â† Preparada para URLs Firebase (Nuevo 08/03)
    └── detalle_trabajo/
        ├── detalle_info_card.dart
        ├── detalle_resumen_final.dart
        ├── detalle_seccion_presupuestos.dart
        └── dialogos_trabajo.dart      â† Todos los AlertDialogs (borrar, finalizar, valorar)
```
...
> âš ï¸ **CAUSA DE CRASH HISTÓRICO:** Usando `popUntil(ModalRoute.withName('/dashboard'))` con rutas
...
### Fase 3: Despliegue Local en Red (Móvil Físico) â¬œ
...
### Fase 5: Documentación y Defensa â¬œ
...
- **Sección de Reseñas:** Implementada una lista visual que muestra las últimas valoraciones de los clientes con estrellas (â­).
...
- **âš ï¸ ATENCIÓN:** La refactorización introdujo errores de compilación que se resolvieron durante la sesión. Los imports correctos son `com.fixfinder.modelos.enums.EstadoTrabajo` y `com.fixfinder.modelos.enums.EstadoPresupuesto`. La firma del servicio de cancelar es `cancelarTrabajo(Integer, String)` → siempre pasar motivo.
...
- Animación añadida al botón `btnRefresh`: `RotateTransition` (360Â°, 0.5s) + `ScaleTransition` (1→0.85→1, 0.5s) en paralelo mediante `ParallelTransition`. Se ejecuta cada vez que se pulsa el botón.
...
### ðŸ¤” DEBATE: Â¿Refactorización primero o Firebase Fotos primero?
...
## ðŸ“ PRÓXIMOS PASOS (SESIÓN SIGUIENTE)
...
### ðŸŒ©ï¸ Fase B: Despliegue AWS (Producción)
...
#### ðŸ”¨ Refactorización del código (Java)
...
SESIÓN 02/04/2026 — Arquitectura "Smart Main", Semáforos y God Mode

### Objetivo de la sesión
Consolidar el proyecto en un único código fuente capaz de operar en **Local (Docker)** y **Nube (AWS)** mediante un interruptor lógico, mejorando la UX con indicadores de estado y restaurando herramientas de test.

### 1. Arquitectura "Smart Switch"
Se ha eliminado la duplicidad de ramas para despliegue:
- **Centralización (Java):** Se utiliza `GlobalConfig.java` como única fuente de verdad para IPs, puertos y credenciales RDS. El booleano `MODO_NUBE` propaga el cambio a todo el sistema (Servidor, Dashboard y Herramientas).
- **Reactividad (Flutter):** El archivo `.env` ahora distingue entre `ENVIRONMENT=LOCAL` y `ENVIRONMENT=NUBE`, inyectando la IP de la instancia EC2 o `10.0.2.2` según corresponda.

### 2. Semáforos de Conexión (Indicadores de Estado)
Implementación de un sistema de feedback visual en tiempo real:
- **Lógica de Colores:** Lógica de Colores: Azul (Local), Gris (Iniciando AWS), Verde (Conexión AWS Exitosa).
- **Asincronía:**
  ...
  - En **Flutter (App)**, se implementó un `ping()` asíncrono en `initState` que actualiza el estado del widget mediante un `ValueNotifier` o `setState`.

### 3. Recuperación y Modernización de Herramientas (God Mode)
Se han rescatado del historial de Git (`c828544`) las herramientas de simulación borradas accidentalmente:
- **`TestPanel`**: Panel de control de bajo nivel para depurar el protocolo de red.
- **`Simulador E2E (God Mode)`**: Herramienta para simular flujo completo (Presupuesto -> Asignación -> Factura -> Pago) sin necesidad de múltiples dispositivos.
- **Mejora:** Se han movido a la carpeta `com.fixfinder.TestPanel` y se han refactorizado para usar `GlobalConfig`. Ahora el "God Mode" también funciona contra AWS.

### 4. Seguridad en el Sembrado (DbClean)
El seeder de la base de datos ahora es "entorno-consciente":
- Detecta si está en modo Nube. Si es así, pide confirmación/advierte y limpia Firebase Storage.
- Si está en modo Local, omite la limpieza de la nube para proteger los archivos reales de producción.
...
## App Movil (Flutter)
- [ ] **Gestión de Perfil:** Habilitar edición de datos personales (teléfono, dirección) en `perfil_pantalla.dart`. Requiere la implementación de la nueva acción `ACTUALIZAR_DATOS_PERSONALES` en el servidor.
...
- [ ] **Actualización Real-Time (Push Real):** Reemplazar el polling de 15s por una escucha reactiva de eventos vía socket.
    - **Técnica:** El `SocketService` debe exponer un `Stream` de eventos globales que el `TrabajoProvider` escuchará para añadir/actualizar elementos sin recargar la vista.
...
- [ ] **Tema de Colores (Centralización):** Extraer todos los colores hardcodeados a `lib/theme/app_theme.dart` usando `ThemeData`. 
    - **Técnica:** Definir un `colorScheme` unificado y usar `Theme.of(context)` en todos los widgets para permitir cambios globales de diseño.
- [ ] **Documentación Explicativa:** Añadir comentarios didácticos línea a línea en todas las clases (`lib/`), explicando el flujo de datos y la lógica técnica para la defensa del proyecto.
...
- [ ] **Registro de Empresas:** Implementar el formulario de alta y la validación visual para nuevos gerentes/empresas desde el lanzador (la lógica de BCrypt ya está operativa en el backend).
- [ ] **Tematización (CSS Centralizado):** Crear `src/main/resources/css/style.css` para unificar el diseño (Dark Mode / Glassmorphism) y eliminar los `setStyle()` del código Java.
...
- [ ] **Sistema de Broadcaster (Push):** Implementar una arquitectura de **Observer** en el servidor para notificar eventos en tiempo real.
    - **Técnica:** Clase `SesionManager` que mantenga un `Set<GestorConexion>` de hilos activos. Al procesar un cambio (nuevo trabajo/presupuesto), iterar y emitir el JSON de aviso a los clientes interesados.
